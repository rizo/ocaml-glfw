(* OASIS_START *)
(* DO NOT EDIT (digest: 8c698b27a5e4b4ca2665fca80816a2ce) *)
...
let package_default =
  {
     MyOCamlbuildBase.lib_ocaml = [("glfw", ["src"], [])];
     lib_c = [];
     flags =
       [
          (["oasis_library_glfw_cclib"; "link"],
            [(OASISExpr.EBool true, S [A "-cclib"; A "-lglfw3"])]);
          (["oasis_library_glfw_cclib"; "ocamlmklib"; "c"],
            [(OASISExpr.EBool true, S [A "-lglfw3"])])
       ];
     includes = [("test", ["src"])]
  }
  ;;
